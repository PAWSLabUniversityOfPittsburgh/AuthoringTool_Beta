...
//								}
							...
							else if (vf.getInitializer() instanceof ClassInstanceCreation)
							{
								ClassInstanceCreation ci = (ClassInstanceCreation) vf.getInitializer();
								if (ci.getType().toString().equals("String"))
								{
									List<Expression> exp = ci.arguments();
									for (Expression e : exp)
										if (e instanceof StringLiteral)
											addConcept("StringInitializationStatement");
								}
								else if (ci.getType().isParameterizedType())
									addConcept("GenericObjectCreationStatement");
								else
									addConcept("ObjectCreationStatement");
								addConcept("ConstructorCall");			 					
								//if (ci.getType().toString().equals("String") == false)
