...
%>
 ...
 <form class="form-horizontal" role="form" name="myForm">
	<div class="form-group">
    	<label for="scope" class="col-sm-3 control-label">Question:</label>
	    <div class="col-sm-9">
			<select class="form-control" name="questionList" id="questionList" onChange="Javascript:changelist();">
...
<%
				...
				try {
					...
					if (isConnectedToDB(conn)) {
						...
						int row = 0;
						if (question != null) {
							out.write("<option value='-1'>Please select the question </option>");
						} else {
...
public Connection getConnectionToTreemap()
{
	...
	{
		Class.forName(this.getServletContext().getInitParameter("db.driver"));
		tempConn = DriverManager.getConnection(this.getServletContext().getInitParameter("db.treemapURL"),this.getServletContext().getInitParameter("db.user"),this.getServletContext().getInitParameter("db.passwd"));
	}catch (Exception e) {
		e.printStackTrace();
	}
	return tempConn;
