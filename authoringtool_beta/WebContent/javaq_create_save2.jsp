...
<%
	...
	connection = DriverManager.getConnection(this.getServletContext().getInitParameter("db.webexURL"),
			this.getServletContext().getInitParameter("db.user"),
			this.getServletContext().getInitParameter("db.passwd"));
	...
	try {
		...
		if (importclassesnames != null && importclassesvals != null && importclassesnames.length() > 0
			...
			for (int i = 0; i < splitResult1.length; i++) {
				if (!splitResult1[i].equals("") && splitResult1[i].length() > 0
						&& !splitResult1[i].equalsIgnoreCase("null") && !splitResult1[i].equals("\"\"")
						&& !splitResult1[i].equals("''") && !splitResult2[i].equals("")
						&& splitResult2[i].length() > 0 && !splitResult2[i].equalsIgnoreCase("null")
						&& !splitResult2[i].equals("\"\"") && !splitResult2[i].equals("''")) {
			...
			System.out.println("importClassName size:" + importClassNames.size() + ",importClassVal size:"
					+ importClassVals.size());
		}
		//}hyun
		...
		rs1 = statement.executeQuery(
				"SELECT id FROM ent_group where name = '" + userBean.getGroupBean().getName() + "' ");
		while (rs1.next()) {
			gid = rs1.getString(1);
		}
		...
		if (privacy2.equals("private")) {
			String command = "insert into ent_jquiz (AuthorID,GroupID,Title,Description,Code,MinVar,MaxVar,AnsType,Privacy,rdfID,QuesType)"
					+ " values ('" + uid + "','" + gid + "','" + (title2_1) + "','" + (description2_1) + "','"
					+ quizcode + "','" + minvar + "','" + maxvar + "','" + anstype + "','0','" + rdfID + "','"
					+ questiontype + "') ";
			statement.executeUpdate(command);

			String MaxID = "";
			rs3 = statement
					.executeQuery("SELECT MAX(LAST_INSERT_ID(QuizID)) AS LastID FROM ent_jquiz WHERE AuthorID='"
							+ uid + "' and GroupID='" + gid + "' and Title='" + (title2_1)
							+ "' and Description='" + (description2_1) + "' and Code='" + quizcode
							+ "' and MinVar='" + minvar + "' and MaxVar='" + maxvar + "' and AnsType='"
							+ anstype + "' and Privacy='0' and rdfID='" + rdfID + "' and QuesType='"
							+ questiontype + "';");
			while (rs3.next())
				MaxID = rs3.getString(1);

			String command1 = "insert into rel_question_quiz (QuestionID,QuizID) values ('" + question + "','"
					+ MaxID + "')";
			...
			String command = "insert into ent_jquiz (AuthorID,GroupID,Title,Description,Code,MinVar,MaxVar,AnsType,Privacy,rdfID,QuesType)"
					+ " values ('" + uid + "','" + gid + "','" + (title2_1) + "','" + (description2_1) + "','"
					+ quizcode + "','" + minvar + "','" + maxvar + "','" + anstype + "','1','" + rdfID + "','"
					+ questiontype + "') ";
			statement.executeUpdate(command);
			...
			rs3 = statement
					.executeQuery("SELECT MAX(LAST_INSERT_ID(QuizID)) AS LastID FROM ent_jquiz WHERE AuthorID='"
							...
							+ "' and Description='" + (description2_1) + "' and Code='" + quizcode
							+ "' and MinVar='" + minvar + "' and MaxVar='" + maxvar + "' and AnsType='"
							+ anstype + "' and Privacy='1' and rdfID='" + rdfID + "' and QuesType='"
							+ questiontype + "';");
			while (rs3.next()) {
				MaxID = rs3.getString(1);
			}
